  ...
  const validateInputs = (loginType: 'email' | 'card'): boolean => {
    ...
    if (loginType === 'email') {
      if (!credentials.email) {
        ...
        isValid = false;
      } else if (credentials.email.includes('@') && !validateEmail(credentials.email)) {
    ...
    try {
      ...
      if (loginType === 'email') {
        if (validateEmail(credentials.email)) {
          result = await loginOptions.loginWithEmail(credentials.email, credentials.password, '/');
        } else {
          result = await loginOptions.loginWithUsername(credentials.email, credentials.password);
        }
      } else {
        result = await loginOptions.loginWithCard(credentials.employeeCard);
      }
          ...
          <Card className="border-0 shadow-xl">
            ...
            <CardContent className="space-y-6">
              ...
              <Tabs
                defaultValue="email"
                className="w-full"
                onValueChange={value => setActiveTab(value as 'email' | 'card')}
              >
                ...
                <TabsContent value="card" className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={e => handleInputChange('employeeCard', e.target.value)}
                      className="h-11"
                      aria-invalid={!!errors.employeeCard}
                    />
                    {errors.employeeCard && <p className="text-sm text-red-600">{errors.employeeCard}</p>}
                  </div>
